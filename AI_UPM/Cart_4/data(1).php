...
<?php
require '../config.php';
$stmt = $conn->prepare("SELECT * FROM Cart_Table WHERE Cart_ID=4");
$stmt->execute();
$result = $stmt->get_result();
$row = $result->fetch_assoc();
if (!$row) {
    header("Location: /AI_Neom/Cart_4/home.php");
    die;
}
?>
<?php
    ...
    <div class="row">
        ...
        <div class="table-responsive mt-5">
            <table class="table table-bordered table-striped text-center bg-light">
                <thead>
                    ...
                    <tr>
                        ...
                        <th>Total Price</th>
                        <th>
                            <a href="/AI_Neom/Cart_4/action.php?clear=all" class="badge-danger badge p-1"
                ...
                <tbody>
                    ...
                    // display all items in cart table
                    require '../config.php';
                    $stmt = $conn->prepare("SELECT * FROM InStore_Table WHERE Cart_ID=4");
                    $stmt->execute();
                    $result = $stmt->get_result();
                    $grand_total = 0;
                    ...
                    <tr>
                        ...
                        <td>
                            <a href="/AI_Neom/Cart_4/action.php?remove=<?= $row['Item'] ?>" class="text-danger lead"
            ...
            <?php
                    // display all items in cart table
                    require '../config.php';
                    $stmt = $conn->prepare("SELECT * FROM Recommendation WHERE Cart_ID=4");
                    $stmt->execute();
                    $result = $stmt->get_result();
