...
bImg = Image.open(f"{imagesFileDirectory}/backgroundOUT.jpg")
...
def loopCapture():
    with Image.open(f"{imagesFileDirectory}/ThePicOUT.jpg") as mImg:
        mResizedImage = mImg.resize((picWidth, picHeight))
    with Image.open(f"{imagesFileDirectory}/backgroundOUT.jpg") as bgImg:
